  ...
  return (
    <div className="mb-8">
      ...
      <div className="space-y-2">
        {logline && (
          <p className="text-zinc-400 text-lg">{logline}</p>
        )}
        {theme && (
      ...
      >
        ...
        {isSaving && <span className="text-zinc-500 ml-2">(saving...)</span>}
      </button>

      {/* Expandable edit section */}
      {isExpanded && (
        <div className="mt-4 space-y-4 bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div>
            <label className="block text-sm text-zinc-400 mb-1">Logline</label>
            <textarea
              value={logline}
              onChange={(e) => handleFieldChange('logline', e.target.value, setLogline)}
              placeholder="One paragraph concept for the series..."
              className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm resize-none focus:border-indigo-500 focus:outline-none"
              rows={2}
            />
          </div>

          <div>
            <label className="block text-sm text-zinc-400 mb-1">Central Theme</label>
            <input
              type="text"
              value={theme}
              onChange={(e) => handleFieldChange('central_theme', e.target.value, setTheme)}
              placeholder="The core thematic exploration..."
              className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
            />
          </div>
